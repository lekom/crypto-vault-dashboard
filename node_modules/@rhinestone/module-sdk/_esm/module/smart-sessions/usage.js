...
import { isAccount } from '../../account/index.js';
...
export const getVerifySignatureResult = async ({ client, account, sender, hash, signature, }) => {
...
export const getAccountEIP712Domain = async ({ client, account, }) => {
...
export const encodeUseOrEnableSmartSessionSignature = async ({ account, client, permissionId, signature, enableSessionData, }) => {
    ...
    return sessionEnabled
        ? encodePacked(['bytes1', 'bytes32', 'bytes'], [SmartSessionMode.USE, permissionId, signature])
...
export const getEnableSessionDetails = async ({ sessions, sessionIndex, enableMode, account, clients, enableValidatorAddress, permitGenericPolicy = false, permitAdminAccess = false, ignoreSecurityAttestations = false, }) => {
    const chainDigests = [];
    const chainSessions = [];
