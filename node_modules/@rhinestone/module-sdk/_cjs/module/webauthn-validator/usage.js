"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.getWebauthnValidatorMockSignature = exports.getWebauthnValidatorSignature = void 0;
const viem_1 = require("viem");
const utils_1 = require("./utils");
const getWebauthnValidatorSignature = ({ webauthn, signature, usePrecompiled = false, }) => {
    const { authenticatorData, clientDataJSON, typeIndex } = webauthn;
    let r;
    let s;
    if (typeof signature === 'string' || signature instanceof Uint8Array) {
        const parsedSignature = (0, utils_1.parseSignature)(signature);
        r = parsedSignature.r;
        s = parsedSignature.s;
    }
    else {
        r = signature.r;
        s = signature.s;
    }
    return (0, viem_1.encodeAbiParameters)([
        { type: 'bytes', name: 'authenticatorData' },
        {
            type: 'string',
            name: 'clientDataJSON',
        },
        {
            type: 'uint256',
            name: 'responseTypeLocation',
        },
        {
            type: 'uint256',
            name: 'r',
        },
        {
            type: 'uint256',
            name: 's',
        },
        {
            type: 'bool',
            name: 'usePrecompiled',
        },
    ], [
        authenticatorData,
        clientDataJSON,
        typeof typeIndex === 'bigint' ? typeIndex : BigInt(typeIndex),
        r,
        s,
        usePrecompiled,
    ]);
};
exports.getWebauthnValidatorSignature = getWebauthnValidatorSignature;
const getWebauthnValidatorMockSignature = () => {
    return '';
};
exports.getWebauthnValidatorMockSignature = getWebauthnValidatorMockSignature;
//# sourceMappingURL=usage.js.map