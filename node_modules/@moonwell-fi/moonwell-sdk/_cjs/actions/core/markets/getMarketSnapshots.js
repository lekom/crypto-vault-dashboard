...
const graphql_js_1 = require("../../morpho/utils/graphql.js");
...
async function fetchMorphoGraphQL(query, operationName, variables) {
    try {
        const response = await axios_1.default.post("https://blue-api.morpho.org/graphql", { query: query, operationName, variables }, { timeout: 5000 });
...
async function fetchIsolatedMarketSnapshots(marketAddress, environment) {
    ...
    const isStkWellMarket = marketConfig &&
    ...
    const variables = {
        options: {
            startTimestamp: dayjs_1.default.utc().subtract(1, "year").unix(),
            interval: "DAY",
        },
    };
    const query = `    
    ...
    if (result) {
        try {
            const markets = result.marketTotalTimeseries.historicalState.borrowAssets.map((borrowAssets, index) => {
                ...
                const totalSupply = (new index_js_1.Amount(BigInt(supplyAssets.y), Number(loanDecimals)).value *
                    loanTokenPrice) /
                    collateralTokenPrice;
                const totalBorrows = new index_js_1.Amount(BigInt(borrowAssets.y), Number(loanDecimals)).value;
                const totalLiquidity = (new index_js_1.Amount(BigInt(liquidityAssets.y), Number(loanDecimals))
                    .value *
                    loanTokenPrice) /
                    collateralTokenPrice;
                return {
...
async function fetchIsolatedMarketSnapshotsSubgraph(marketAddress, environment) {
    ...
    if (isStkWellMarket) {
      ...
      `;
            const wellResult = await (0, graphql_js_1.getSubgraph)(environment, wellQuery);
    ...
    if (result) {
        try {
            const markets = result.marketDailySnapshots.map((item) => {
                ...
                const totalSupplyInLoanToken = new index_js_1.Amount(BigInt(supplyAssets), Number(supplyDecimals)).value;
                ...
                const totalBorrows = new index_js_1.Amount(BigInt(borrowAssets), Number(supplyDecimals)).value;
