...
async function getMorphoMarketsData(params) {
    ...
    const fulfilledMarketsInfo = marketInfoSettlements.flatMap((s, i) => s.status === "fulfilled"
        ...
        const markets = marketsInfo.map((marketInfo) => {
            ...
            if (collateralToken.symbol === "stkWELL") {
                const wellMarketInfo = marketsInfo.find((mi) => {
                    const wellMarketConfig = Object.values(environment.config.morphoMarkets).find((item) => item.id.toLowerCase() === mi.marketId.toLowerCase() &&
                ...
                if (wellMarketInfo) {
                    ...
                    if (wellMarketConfig &&
                        ...
                        wellPrice = new amount_js_1.Amount(wellMarketInfo.collateralPrice, 18).value;
                        if (wellPrice === 0) {
                            const wellLoanToken = environment.config.tokens[wellMarketConfig.loanToken];
                            const wellLoanPrice = new amount_js_1.Amount(wellMarketInfo.loanPrice, 18)
                                .value;
                            const wellOraclePrice = new amount_js_1.Amount(BigInt(wellMarketInfo.oraclePrice), 36 + wellLoanToken.decimals - 18).value;
                            wellPrice = wellLoanPrice * wellOraclePrice;
                        }
                    }
                    else if (wellMarketConfig &&
                        wellMarketConfig.loanToken === "WELL") {
                        wellPrice = new amount_js_1.Amount(wellMarketInfo.loanPrice, 18).value;
                        if (wellPrice === 0) {
                            const wellCollateralToken = environment.config.tokens[wellMarketConfig.collateralToken];
                            const wellCollateralPrice = new amount_js_1.Amount(wellMarketInfo.collateralPrice, 18).value;
                            const wellOraclePrice = new amount_js_1.Amount(BigInt(wellMarketInfo.oraclePrice), 36 + 18 - wellCollateralToken.decimals).value;
