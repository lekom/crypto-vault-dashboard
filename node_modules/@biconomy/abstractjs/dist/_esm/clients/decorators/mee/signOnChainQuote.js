import { concatHex, encodeAbiParameters, erc20Abi, zeroAddress } from "viem";
 ...
 * ```
 */
export const prepareExecutableOnChainQuotePayload = async (quoteParams, spender, recipient, version) => {
 ...
 * ```
 */
export const formatSignedOnChainQuotePayload = (quoteParams, _metadata, // This is unused for now. But can be extended in future
hash) => {
    const { quote, trigger } = quoteParams;
...
export const signOnChainQuote = async (client, params) => {
    ...
    const { executablePayload, metadata } = await prepareExecutableOnChainQuotePayload(params.fusionQuote, spender, // In terms of token approval. Spender will be used for approving for SCA
